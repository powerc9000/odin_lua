...
{
	result := f64( lua_tonumberx(L,(i),nil) );
...
{
	len : int = 0;
	//The pointer we get from lua is memory managed by lua.
	//We need to copy it.
	ptr: rawptr = cast(rawptr)lua_tolstring(L, (i), &len);
	dest: rawptr = mem.alloc(len);


	mem.copy(dest, ptr, len);
	return strings.string_from_ptr(transmute(^u8)dest, len);
}	
...
lua_pcall :: proc (L: ^lua_State,n:int,r:int,f:int) -> int
{
